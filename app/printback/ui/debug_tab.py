...
class DebugTab(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        self.lbl_rate = self._make_stat(tr("kpi.events_per_min"), "--")
        self.lbl_obs_24h = self._make_stat(tr("kpi.obs_24h"), "--")
        self.lbl_ch1 = self._make_stat(tr("kpi.ch", ch=1), "--")
        self.lbl_ch6 = self._make_stat(tr("kpi.ch", ch=6), "--")
        self.lbl_ch11 = self._make_stat(tr("kpi.ch", ch=11), "--")
        self.lbl_candidates = self._make_stat(tr("kpi.auto_wl_candidates"), "--")
        ...
        self.rssi_plot = pg.PlotWidget(title=tr("chart.live_rssi"))
        self.rssi_plot.showGrid(x=True, y=True, alpha=0.2)
        self.rssi_plot.setLabel("left", tr("chart.live_rssi_y"))
        self.rssi_plot.setLabel("bottom", tr("chart.live_rssi_x"))
        ...
        log_label = QLabel(tr("rawlog.label"))
    ...
    def _build_devices_table(self) -> None:
        ...
        header = QLabel(tr("devices.header"))
        ...
        cols = [
            ...
            tr("devices.col.mac"),
            tr("devices.col.rssi"),
            tr("devices.col.avg_rssi"),
            tr("devices.col.n_obs"),
            tr("devices.col.first"),
            tr("devices.col.last"),
            tr("devices.col.channel"),
            tr("devices.col.status"),
        ]
    ...
    def on_observation(self, obs: Observation) -> None:
        ...
        line = (
            f"{ts}  fp={obs.fp}  mac={obs.mac}  rssi={obs.rssi:+4d}  "
    ...
    def _populate_devices_table(self, devices: list[LiveDevice]) -> None:
        ...
        for row, d in enumerate(devices):
            ...
            cells = [
                ...
                _NumericItem(d.first_seen, tr("devices.ago_s", s=int(now - d.first_seen))),
                _NumericItem(d.last_seen, tr("devices.ago_s", s=int(now - d.last_seen))),
    ...
    def _status_for(self, d: LiveDevice) -> str:
        ...
        return tr("devices.status.client")
